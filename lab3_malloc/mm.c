...
static void *find_fit(size_t asize)
{
     ...
     */
    void* fit_bp = heap_listp;
    fit_bp = (char*)(fit_bp); /*first 8/1 block*/
    while(GET_SIZE(HDRP(fit_bp)) < asize || GET_ALLOC(HDRP(fit_bp)) == 1){
        if(GET_SIZE(HDRP(fit_bp)) == 0) return NULL;
...
static void place(void *bp, size_t asize)
{
    ...
    else
    {
        ...
        PUT(HDRP(bp), PACK(total_size, 1)); /* flag bit set to 1 */
